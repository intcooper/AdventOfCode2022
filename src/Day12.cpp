...
namespace AdventOfCode
{
    ...
    void Day12::Task2()
    {
        ...
        for (const auto& c : m_candidateStartPos)
        {
            ...
            if ((shortestPath.size() > 0) && (shortestPathLength > shortestPath.size()))
            {
    ...
    std::vector<Point> Day12::Explore(const Point& start, const Point& end)
    {
        ...
        constexpr std::array<Point, 4> validDirs = {Point{-1, 0}, {+1, 0}, {0, -1}, {0, +1}};
        ...
        while (!open.empty())
        {
            ...
            for (const auto& d : validDirs)
            {
                ...
                if (!((next.y >= 0) && (next.y < m_map.size()) && (next.x >= 0) && (next.x < m_map[0].size())))
                {
                    continue;
                }
                ...
                const auto h = std::abs(m_endPos.y - next.y) + std::abs(m_endPos.x - next.x); // taxicab distance
                const auto f = g + h;
