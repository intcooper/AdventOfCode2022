...
void Day13::Task1()
{
    ...
    for (std::string line; std::getline(m_inputFile, line);)
    {
        if (line.empty())
        {
            ...
            auto res =
                ComparePackets(TextToPacket(strLeft), TextToPacket(strRight));
...
void Day13::Task2()
{
    ...
    std::ranges::sort(packets, [this](auto a, auto b) {
        return ComparePackets(a, b) == true;
    });
                                          ...
                                          const Packet& right)
{
    ...
    while (index <= maxIndex)
    {
        ...
        else
        {
            ...
            {
                auto res = ComparePackets(left.GetList(index),
                                          newRight.GetList(index));
