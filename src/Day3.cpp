...
namespace AdventOfCode
{
	...
	void Day3::Task1()
	{
		...
		for (std::string line; std::getline(m_inputFile, line); )
		{
			...
			{
				if (std::find(line.begin() + middle, line.end(), line[i]) != line.end())
				{
	...
	void Day3::Task2()
	{
		...
		for (std::string line; std::getline(m_inputFile, line); )
		{
			if (lineIndex % 3 == 0)
			{
				...
				{
					if (std::find(rucksacks[1].begin(), rucksacks[1].end(), rucksacks[0][i]) != rucksacks[1].end())
					{
						if (std::find(rucksacks[2].begin(), rucksacks[2].end(), rucksacks[0][i]) != rucksacks[2].end())
						{
