...
namespace AdventOfCode
{
	...
	void Day7::Task1()
	{
		...
		auto sizeOfDirs = std::accumulate(dirs.begin(), dirs.end(), 0, [](auto a, auto b) { return a += b.second; });
	...
	void Day7::ParseInputFile()
	{
		...
		for (std::string line; std::getline(m_inputFile, line); )
		{
				...
				if (parts[1] == "cd")
				{
					...
					else
					{
						...
						else
						{
							if (currentDir != nullptr)
							{
								currentDir = currentDir->GetDirectory(parts[2].data());
							}
						}
					}
				}
			...
			else
			{
				if (parts[0] == "dir")
				{
					currentDir->AddDirectory(std::make_unique<Directory>(parts[1].data(), currentDir));
				}
				else
				{
					currentDir->AddFile(std::make_unique<File>(parts[1].data(), static_cast<uint32_t>(std::stoi(parts[0].data()))));
				}
			}
		}
	}
