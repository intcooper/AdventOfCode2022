...
namespace AdventOfCode
{
    ...
    void Day11::RunTask(int numOfRounds, std::function<uint64_t(uint64_t)> reduceWorry)
    {
    ...
    void Day11::ParseInputFile()
    {
        ...
        for (std::string line; std::getline(m_inputFile, line); )
		{
            ...
            if (parts[0] == "  Starting items")
            {
                ...
                std::transform(subparts.begin(), subparts.end(), std::back_inserter(items), [](auto i)
                {
            ...
            if (parts[0] == "  Operation")
            {
                ...
                if (subparts[5].find_first_not_of("0123456789") == std::string::npos)
                {
                    // is a number
                ...
                else
                {
                    ...
                    operation = std::bind(oper, std::placeholders::_1, std::placeholders::_1);
                }
            ...
            {
                auto subparts = Utils::Split(parts[1], ' ');
                onTestPositive = std::bind([&m_monkeys = m_monkeys](auto worry, auto monkeyId){ m_monkeys[monkeyId].AddItem(worry); }, std::placeholders::_1, std::stoi(subparts[4].data()));
            ...
            {
                auto subparts = Utils::Split(parts[1], ' ');
                onTestNegative = std::bind([&m_monkeys = m_monkeys](auto worry, auto monkeyId){ m_monkeys[monkeyId].AddItem(worry); }, std::placeholders::_1, std::stoi(subparts[4].data()));
            ...
            if (parts[0].empty())
            {
               m_monkeys.emplace_back(items, operation, testDivisor, onTestPositive, onTestNegative);
               items.clear();
               operation = nullptr;
