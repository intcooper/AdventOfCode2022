...
namespace AdventOfCode
{
	...
	void Day2::Task1()
	{
		...
		for (std::string line; std::getline(m_inputFile, line); )
		{
			...
			{
				if (WinCombinations[static_cast<size_t>(opponentPlay) - 1] == playerPlay)
				{
					outcome = 6;
				}
			}
	...
	void Day2::Task2()
	{
		...
		for (std::string line; std::getline(m_inputFile, line); )
		{
			...
			else
			{
				if (outcome == 0)
				{
					playerPlay = LossCombinations[static_cast<size_t>(opponentPlay) - 1];
				}
				else
				{
					playerPlay = WinCombinations[static_cast<size_t>(opponentPlay) - 1];
				}
			}
